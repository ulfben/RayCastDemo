...
#define STBI_NO_HDR //SDL_Surface doesn't support HDR anyway
...
SDL_Surface* STBIMG_CreateSurface(unsigned char* pixelData, int width, int height, int bytesPerPixel, SDL_bool freeWithSurface);
...
// Returns NULL on error, use SDL_GetError() to get more information.
SDL_Texture* STBIMG_LoadTextureFromMemory(SDL_Renderer* renderer, const unsigned char* buffer, int length);
...
SDL_Texture* STBIMG_LoadTexture_RW_noSeek(SDL_Renderer* renderer, SDL_RWops* src, int freesrc);

#endif // SDL__STBIMAGE_H

// ############# Below: Implementation ###############
#ifdef SDL_STBIMAGE_IMPLEMENTATION
// make stb_image use SDL_malloc etc, so SDL_FreeSurface() can SDL_free() the data allocated by stb_image
...
SDL_Surface* STBIMG_Load_RW_noSeek(SDL_RWops* src, int freesrc){
	...
	Sint64 fileSize = 0;
	std::size_t fileSizeClean = 0;  //avoid error C2362: initialization of 'fileSizeClean' is skipped by 'goto end'
	SDL_Surface* ret = NULL;
	if(src == NULL){
...
SDL_Texture* STBIMG_CreateTexture(SDL_Renderer* renderer, const unsigned char* pixelData, int width, int height, int bytesPerPixel){
